...
export const Route = createFileRoute("/auth/signup")({
  head: () => ({
    meta: [
      { title: "Register — SAGIP" },
      {
        name: "description",
        content: "Create your SAGIP account. Required: valid government ID and 18+ years of age.",
      },
    ],
  }),
...
function SignupPage() {
  ...
  const next = async () => {
    ...
    if (step === 1 && idFile && idFile.size > 5 * 1024 * 1024) {
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-xl sm:p-8">
          ...
          <form onSubmit={handleOtpSubmit} className="mt-6 space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="otp" className="text-left block text-xs font-semibold">
                Verification Code
              </Label>
              <Input
                id="otp"
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 lg:px-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        All applicants must be 18 years or older and provide a valid government-issued ID.
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-6">
        ...
        {step === 1 && (
          <Fieldset
            ...
            description="Upload a clear photo of one valid government ID. You must be 18+ to register."
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="ID type" error={errors.idType?.message}>
                <Select
                  onValueChange={(v: any) => setValue("idType", v, { shouldValidate: true })}
                  value={watch("idType") ?? ""}
                >
                  <SelectTrigger>
            ...
            <label className="flex cursor-pointer items-center justify-center gap-3 rounded-md border-2 border-dashed border-border bg-paper px-4 py-8 text-sm text-muted-foreground hover:border-primary hover:bg-accent">
              ...
              {idFile ? (
                <>
                  <CheckCircle2 className="h-5 w-5 text-relief" />
                  <span className="font-medium text-foreground">{idFile.name}</span>
                </>
              ) : (
                <>
                  <Upload className="h-5 w-5" /> Click to upload (JPG, PNG, PDF)
                </>
              )}
            </label>
          </Fieldset>
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Confirm password" error={errors.confirm?.message}>
                <Input
                  type="password"
                  {...register("confirm", { onBlur: () => trigger("confirm") })}
                />
              </Field>
            </div>
          ...
          >
            <label className="flex items-start gap-3 rounded-md border border-border bg-paper p-3 text-sm">
              ...
              <span className="text-muted-foreground">
                ...
                />
                , including acceptable-use policies and the audit logging of all donations and
                assistance requests.
              </span>
            </label>
            {errors.acceptTerms && (
            ...
            <label className="flex items-start gap-3 rounded-md border border-border bg-paper p-3 text-sm">
              ...
              />
              <span className="text-muted-foreground">
                <strong className="text-ink">Data Privacy Consent (RA 10173):</strong> I freely
                ...
                verification, disaster relief operations, donor acknowledgment, and audit purposes,
                ...
                >
                  SAGIP Privacy Notice
                </a>
                . I understand my rights to access, correct, and request deletion of my data under
