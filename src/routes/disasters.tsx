...
const qo = queryOptions({
  ...
  queryFn: async () => {
    const { data } = await supabase
      .from("disasters")
      .select(
        "id,name,city,severity,status,affected_families,required_funding,raised_amount,occurred_at,disaster_categories(name,slug)",
      )
...
export const Route = createFileRoute("/disasters")({
  head: () => ({
    meta: [
      { title: "Disaster campaigns — SAGIP" },
      {
        name: "description",
        content: "Active and recent disaster campaigns tracked by the City Government DRRM Office.",
      },
    ],
  }),
        ...
        <section className="border-b border-border bg-paper py-12">
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            ...
            <h1 className="mt-2 font-display text-4xl font-semibold">
              Disaster campaigns under response
            </h1>
            <p className="mt-2 max-w-2xl text-muted-foreground">
              A live list of disaster campaigns the city is currently responding to, with funding
              progress for each.
            </p>
          </div>
        </section>
                ...
                {data.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-12 text-center text-muted-foreground">
                      No disaster campaigns on record.
                    </td>
                  </tr>
                )}
                {data.map((d: any) => (
