...
const schema = z.object({
  ...
  affected_individuals: z.coerce
    .number({ invalid_type_error: "Required" })
    .int()
    .min(1, "Must affect at least 1 person")
    .max(1_000_000),
  estimated_damage_cost: z.coerce
    .number({ invalid_type_error: "Required" })
    .min(0)
    .max(1_000_000_000),
...
function RequestPage() {
  ...
  const { data: categories } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("disaster_categories").select("id,name").order("name")).data ?? [],
  });
  ...
  const disasterDescription = watch("disaster_description") || "";
  const wordCount =
    disasterDescription.trim() === ""
      ? 0
      : disasterDescription.trim().split(/\s+/).filter(Boolean).length;
  ...
  const onFilesPicked = (list: FileList | null) => {
    ...
    for (const f of Array.from(list)) {
      if (!ALLOWED.includes(f.type)) {
        toast.error(`${f.name}: only PDF, PNG, JPG, WEBP allowed`);
        continue;
      }
      if (f.size > MAX_SIZE) {
  ...
  const onSubmit = async (vals: FormVals) => {
    ...
    try {
      ...
      toast.error((e as Error | null)?.message ?? "Failed to submit request");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <DashShell
      title="Request disaster assistance"
      subtitle="Submit a verified request for SAGIP relief funding. All submissions are reviewed by the SAGIP DRRM Office."
    >
        ...
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-warning/40 bg-warning/10 p-4 text-sm">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              You may still submit a request, but it will only be processed after SAGIP verifies
              your uploaded valid ID. Verification typically takes 1 business day.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6 rounded-xl border border-border bg-card p-6 sm:p-8">
          <section>
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Select
                  value={watch("disaster_id") || "__none__"}
                  onValueChange={(v) => {
              ...
              <div>
                ...
                >
                  <SelectTrigger id="cat" className="mt-1.5">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-xs ${wordCount > 300 ? "font-semibold text-destructive" : "text-muted-foreground"}`}
                >
                  {wordCount}/300 words
                </span>
              </div>
              <Textarea
                ...
                rows={5}
                className="mt-1.5"
                placeholder="When did it happen, what was destroyed, who is affected, and what immediate assistance do you need?"
                {...register("disaster_description")}
              />
              {errors.disaster_description && (
                <p className="mt-1 text-xs text-destructive">
                  {errors.disaster_description.message}
                </p>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                ...
                >
                  <SelectTrigger id="city" className="mt-1.5">
                    <SelectValue placeholder="Select Metro Manila city" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                  ...
                  return (
                    ...
                    >
                      <SelectTrigger id="brgy" className="mt-1.5">
                        <SelectValue
                          placeholder={
                            selectedCity ? "Select your barangay" : "Select a city first"
                          }
                        />
                      </SelectTrigger>
                ...
                {errors.barangay && (
                  <p className="mt-1 text-xs text-destructive">{errors.barangay.message}</p>
                )}
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <Input
                  id="exact"
                  className="mt-1.5"
                  {...register("exact_location")}
                  placeholder="House #, street, sitio, landmark"
                />
                {errors.exact_location && (
                  <p className="mt-1 text-xs text-destructive">{errors.exact_location.message}</p>
                )}
              </div>
            </div>
          </section>
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <Input
                  ...
                  className="mt-1.5"
                  placeholder="e.g. 25"
                  {...register("affected_individuals")}
                />
                {errors.affected_individuals && (
                  <p className="mt-1 text-xs text-destructive">
                    {errors.affected_individuals.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  className="mt-1.5"
                  placeholder="0.00"
                  {...register("estimated_damage_cost")}
                />
                {errors.estimated_damage_cost && (
                  <p className="mt-1 text-xs text-destructive">
                    {errors.estimated_damage_cost.message}
                  </p>
                )}
              </div>
              <div>
                ...
                {errors.requested_amount && (
                  <p className="mt-1 text-xs text-destructive">{errors.requested_amount.message}</p>
                )}
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Upload photos of damage, barangay certifications, or proof of residency. Max{" "}
              {MAX_FILES} files, 10 MB each. PDF, PNG, JPG, WEBP.
            </p>

            <label
              htmlFor="files"
              className="mt-4 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border bg-paper p-8 text-center hover:border-primary/40"
            >
            ...
            {files.length > 0 && (
              <ul className="mt-3 space-y-1.5">
                {files.map((f, i) => (
                  <li
                    key={i}
                    className="flex items-center justify-between rounded border border-border bg-paper px-3 py-2 text-sm"
                  >
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-6 text-sm">
            ...
            <ol className="mt-3 space-y-3 text-muted-foreground">
              <li>
                <span className="font-semibold text-ink">1. Triage.</span> SAGIP Operations reviews
                your submission within 24 hours.
              </li>
              <li>
                <span className="font-semibold text-ink">2. Verification.</span> A field officer
                confirms the disaster occurred at the reported location.
              </li>
              <li>
                <span className="font-semibold text-ink">3. Release.</span> Approved funds are
                released with documented proof.
              </li>
            </ol>
          </div>
          <div className="rounded-xl border border-warning/40 bg-warning/10 p-6 text-sm">
            ...
            <p className="mt-1 text-muted-foreground">
              Submitting fraudulent disaster claims is punishable under RA 10121 (DRRM Act) and the
              Revised Penal Code. All requests are cross-checked against barangay records.
            </p>
          </div>
        </aside>
      </form>
