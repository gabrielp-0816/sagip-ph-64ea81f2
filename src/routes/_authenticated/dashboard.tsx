...
function Dashboard() {
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase
          .from("disasters")
          .select(
            "id,name,city,severity,affected_families,required_funding,raised_amount,created_by,status,occurred_at,created_at,closure_requested,closure_requested_at,disaster_categories(name)",
          )
          .eq("status", "active")
        ...
        supabase
          .from("disasters")
          .select(
            "id,name,city,severity,affected_families,required_funding,raised_amount,created_by,status,disaster_categories(name)",
          )
          .neq("status", "active")
          .order("created_at", { ascending: false })
          .limit(20),
        supabase
          .from("donations")
          .select("id,amount,created_at,disasters(name)")
          .eq("donor_id", uid)
          .order("created_at", { ascending: false })
          .limit(5),
        supabase
          .from("fund_requests")
          .select("id,requester_id,disaster_description,status,requested_amount,created_at")
          .eq("requester_id", uid)
          .order("created_at", { ascending: false })
          .limit(5),
      ]);
      ...
      const creatorIds = Array.from(
        new Set(
          [
            ...rawDisasters.map((d: any) => d.created_by),
            ...rawInactive.map((d: any) => d.created_by),
          ].filter(Boolean),
        ),
      );
      ...
      for (const a of allocations.data ?? []) {
        ...
        releasedByDisaster[a.disaster_id] =
          (releasedByDisaster[a.disaster_id] ?? 0) + Number(a.released_amount);
      }
  ...
  return (
    <DashShell
      ...
      subtitle="Live view of city disaster relief fund operations."
    >
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          label="Total donations received"
          value={formatPHP(s?.totalDonations ?? 0)}
          icon={HandHeart}
          accent="relief"
        />
        <KpiCard
          label="Available funds"
          value={formatPHP(s?.availableFunds ?? 0)}
          icon={TrendingUp}
        />
        <KpiCard
          label="Funds released"
          value={formatPHP(s?.fundsReleased ?? 0)}
          icon={Activity}
          accent="gold"
        />
        <KpiCard
          label="Requests pending"
          value={String(s?.fundsPending ?? 0)}
          icon={ShieldAlert}
          accent="warning"
        />
      </div>
        ...
        <ul className="divide-y divide-border">
          {(s?.inactiveDisasters ?? []).length === 0 && (
            <li className="p-10 text-center text-sm text-muted-foreground">
              No inactive disaster campaigns at this time.
            </li>
          )}
              ...
              <li key={d.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto] sm:items-center">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    {d.disaster_categories?.name}
                  </p>
                  <p className="mt-0.5 font-display text-lg font-semibold">{d.name}</p>
                  <p className="text-sm text-muted-foreground">
                    {d.city} · {d.affected_families.toLocaleString()} families affected
                  </p>
                ...
                <div className="flex flex-wrap gap-2 sm:flex-col sm:items-stretch">
                  <span className="inline-flex items-center justify-center rounded-full bg-secondary px-2 py-0.5 text-[10px] font-semibold uppercase capitalize">
                    {d.status}
                  </span>
                </div>
              </li>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border p-5">
            ...
            <Button variant="ghost" size="sm" onClick={() => setOpenDialog("donations")}>
              View all <ArrowRight className="h-3 w-3" />
            </Button>
          </div>
          <ul className="divide-y divide-border">
            ...
            {(s?.myDonations ?? []).map((d: any) => (
              <li key={d.id} className="flex items-center justify-between p-4 text-sm">
                ...
                <p className="font-display font-semibold tabular-nums text-relief">
                  {formatPHP(d.amount)}
                </p>
              </li>
            ))}
        ...
        <section className="rounded-xl border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border p-5">
            ...
            <Button variant="ghost" size="sm" onClick={() => setOpenDialog("requests")}>
              View all <ArrowRight className="h-3 w-3" />
            </Button>
          </div>
          <ul className="divide-y divide-border">
              ...
              <li key={r.id} className="flex items-center justify-between gap-3 p-4 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {timeAgo(r.created_at)} · {formatPHP(r.requested_amount)}
                  </p>
                </div>
      ...
      <DisastersDialog
        open={openDialog === "disasters"}
        onOpenChange={(o) => !o && setOpenDialog(null)}
      />
      <InactiveDisastersDialog
        open={openDialog === "inactive-disasters"}
        onOpenChange={(o) => !o && setOpenDialog(null)}
      />
      <DonationsDialog
        open={openDialog === "donations"}
        onOpenChange={(o) => !o && setOpenDialog(null)}
        uid={uid}
      />
      <RequestsDialog
...
function MyCampaignsSection({ campaigns }: { campaigns: any[] }) {
  ...
  const submitClosure = async () => {
    ...
    setSubmitting(true);
    const { error } = await supabase.rpc("request_campaign_closure", {
      _disaster_id: target.id,
      _reason: reason.trim(),
    });
    setSubmitting(false);
    ...
    <section className="mt-8 rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between border-b border-border p-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Campaigns started from your assistance requests.
          </p>
        </div>
      </div>
      <ul className="divide-y divide-border">
          ...
          return (
            <li key={d.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto] sm:items-center">
              <div>
                ...
                <p className="mt-0.5 font-display text-lg font-semibold">{d.name}</p>
                <p className="text-sm text-muted-foreground">
                  {d.city} · {d.affected_families?.toLocaleString?.() ?? d.affected_families}{" "}
                  families affected
                </p>
                <div className="mt-3 max-w-md">
                  <div className="flex items-baseline justify-between text-xs">
                  ...
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-secondary">
                    <div
                      className="h-full bg-gradient-to-r from-relief to-primary"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
      ...
      <Dialog open={!!target} onOpenChange={(o) => !o && setTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Admins will be notified to review and close{" "}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g. our community has recovered and no longer needs additional aid"
            />
          </div>
          <DialogFooter>
            ...
            <Button onClick={submitClosure} disabled={submitting}>
...
function ActiveCampaignsSection({
  ...
  const [sort, setSort] = useState<
    "date_desc" | "date_asc" | "name_asc" | "name_desc" | "funded_desc"
  >("date_desc");
  ...
  const filtered = useMemo(() => {
    return disasters
      ...
      .sort((a, b) => {
        ...
        if (sort === "funded_desc") {
          const pa =
            a.required_funding > 0 ? Number(a.raised_amount) / Number(a.required_funding) : 0;
          const pb =
            b.required_funding > 0 ? Number(b.raised_amount) / Number(b.required_funding) : 0;
          return pb - pa;
        }
  ...
  return (
    <section className="mt-8 rounded-xl border border-border bg-card">
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">
              {Math.min(visible.length, filtered.length)} of {filtered.length}
            </span>
      ...
      <ul className="divide-y divide-border">
        {disasters.length === 0 && (
          <li className="p-10 text-center text-sm text-muted-foreground">
            No active disaster campaigns at this time.
          </li>
        )}
            ...
            <li key={d.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto] sm:items-center">
              <div className="min-w-0">
                ...
                <p className="mt-1.5 text-xs text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">
                    {d.creator_profile
                      ? `${d.creator_profile.first_name} ${d.creator_profile.last_name}`
                      : "Citizen"}
                  </span>{" "}
              ...
              <div className="flex flex-wrap gap-2 sm:flex-col sm:items-stretch">
                <Button variant="relief" size="sm" asChild>
                  <Link to="/donate" search={{ disaster: d.id } as any}>
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("disasters")
        .select(
          "id,name,city,severity,status,affected_families,required_funding,raised_amount,occurred_at,disaster_categories(name)",
        )
        .eq("status", "active")
        .order("created_at", { ascending: false });
      return data ?? [];
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        {d.city} · {d.affected_families.toLocaleString()} families
                      </p>
                    </div>
...
function InactiveDisastersDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const [sort, setSort] = useState<"date_desc" | "date_asc" | "name_asc" | "name_desc">(
    "date_desc",
  );
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("disasters")
        .select(
          "id,name,city,severity,status,affected_families,required_funding,raised_amount,occurred_at,created_at,disaster_categories(name)",
        )
        .neq("status", "active")
        .order("created_at", { ascending: false });
      return data ?? [];
  ...
  const statusOptions = Array.from(
    new Set(all.map((d: any) => d.status).filter(Boolean)),
  ) as string[];
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="flex items-center gap-1 text-xs text-muted-foreground">
                        <MapPin className="h-3 w-3" />
                        {d.city} · {d.affected_families.toLocaleString()} families ·{" "}
                        {formatDate(d.occurred_at ?? d.created_at)}
                      </p>
                    </div>
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-semibold uppercase capitalize">
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("fund_requests")
        .select(
          "id,requester_id,disaster_description,status,requested_amount,affected_individuals,city,barangay,created_at,disasters(name),disaster_categories(name)",
        )
        .eq("requester_id", uid)
        .order("created_at", { ascending: false });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-hidden p-0">
        <DialogHeader className="border-b border-border p-5">
          ...
          <DialogDescription>
            All relief requests you've submitted and their status.
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-[65vh] overflow-y-auto">
          {q.isLoading && (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs uppercase tracking-wider text-muted-foreground">
                      {r.disaster_categories?.name} · {formatDate(r.created_at)}
                    </p>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {r.barangay}, {r.city} · {r.affected_individuals?.toLocaleString?.() ?? 0}{" "}
                      individuals
                    </p>
                  </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${map[status] ?? "bg-secondary text-muted-foreground"}`}
    >
      {status.replace("_", " ")}
    </span>
  );
