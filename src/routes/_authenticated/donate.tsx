...
function DonatePage() {
  ...
  const onPickProof = (f: File | null) => {
    ...
    if (!PROOF_MIME.includes(f.type)) { toast.error("Proof must be JPG, PNG, WEBP, or PDF"); return; }
    if (f.size > MAX_PROOF_SIZE) { toast.error("Proof must be 5MB or smaller"); return; }
    setProofFile(f);
  };

  const onSubmit = async (vals: FormVals) => {
    if (!profile) { toast.error("Profile not loaded"); return; }
    if (!proofFile) { toast.error("Please upload your proof of payment (e.g. GCash or bank receipt screenshot)"); return; }
    setSubmitting(true);
    const { data: auth } = await supabase.auth.getUser();
    ...
    const up = await supabase.storage.from("donation-proofs").upload(proofPath, proofFile, { contentType: proofFile.type });
    if (up.error) {
      setSubmitting(false);
    ...
    setProofFile(null);
    reset({ disaster_id: vals.disaster_id, payment_method: vals.payment_method, is_anonymous: false, reference_number: "", message: "" });
  };

  return (
    <DashShell title="Donate to disaster relief" subtitle="Your contribution is logged in the SAGIP chain of custody and visible on the Transparency Portal.">
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-xl border border-border bg-card p-6 sm:p-8">
          <section>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Direct your funds to a specific disaster campaign, or contribute to the SAGIP General Fund.</p>
            <div className="mt-4">
          ...
          <section>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Choose how you remitted the funds. The SAGIP Treasury reconciles transactions within 1 business day.</p>
            ...
            <div className="mt-4">
              ...
              <p className="text-xs text-muted-foreground">Upload a screenshot of your GCash / Maya / bank transfer receipt (JPG, PNG, WEBP, or PDF — max 5MB).</p>
              <label htmlFor="proof" className="mt-2 flex cursor-pointer items-center gap-3 rounded-lg border-2 border-dashed border-border bg-paper p-4 text-sm hover:border-primary/40">
                {proofFile ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="text-muted-foreground">Click to upload your payment receipt</span>
                  </>
                )}
              </label>
              <input id="proof" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="sr-only" onChange={(e) => onPickProof(e.target.files?.[0] ?? null)} />
            </div>
          </section>
          ...
          <section>
            ...
            <label className="mt-4 flex items-start gap-2 text-sm text-muted-foreground">
              ...
              <span>List my donation as anonymous on the public Transparency Portal. (SAGIP still keeps your identity for audit and receipt purposes.)</span>
            </label>
          </section>

          <Button type="submit" size="lg" variant="relief" disabled={submitting} className="w-full sm:w-auto">
            {submitting && <Loader2 className="h-4 w-4 animate-spin" />} <HandHeart className="h-4 w-4" /> Record donation{amount > 0 ? ` of ${formatPHP(amount)}` : ""}
          </Button>
        </form>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
              ...
              <li>• Every release is approved by the SAGIP DRRM Council and published on the Transparency Portal.</li>
              <li>• You receive an official receipt by email after SAGIP Treasury reconciles the transaction.</li>
            </ul>
          </div>
          ...
          {selected ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-card/60 p-6 text-sm text-muted-foreground">
              You are contributing to the <strong className="text-ink">SAGIP General Fund</strong>. It bankrolls preparedness training, equipment, and rapid-response stockpiles citywide.
            </div>
          )}
