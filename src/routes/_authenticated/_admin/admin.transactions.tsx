...
function Transactions() {
  ...
  const totals = useMemo(() => {
    ...
    for (const r of filtered)
      r.direction === "donation" ? (inc += Number(r.amount)) : (out += Number(r.amount));
    return { inc, out, net: inc - out };
  }, [filtered]);

  const exportCsv = () => {
    const rows = [
      ["Date", "Type", "Party", "Category", "Reference", "Amount", "Status", "Reference #"],
    ];
    for (const r of filtered) {
      rows.push([
    ...
    >
      <div className="mb-4 grid gap-3 sm:grid-cols-3">
        <Kpi
          ...
          value={formatPHP(totals.inc)}
          icon={ArrowDownCircle}
          accent="relief"
        />
        <Kpi
          label="Outgoing (releases)"
          value={formatPHP(totals.out)}
          icon={ArrowUpCircle}
          accent="gold"
        />
        <Kpi
          ...
          accent={totals.net >= 0 ? "relief" : "warning"}
        />
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="relative max-w-sm flex-1">
          ...
          <Input
            className="pl-9"
            placeholder="Search party, category, reference..."
            value={search}
          ...
          <tbody className="divide-y divide-border">
            ...
            {filtered.map((r) => (
              <tr key={`${r.direction}-${r.id}`} className="hover:bg-secondary/50">
                ...
                <td
                  className={`px-4 py-3 text-right font-semibold tabular-nums ${r.direction === "donation" ? "text-relief" : "text-ink"}`}
                >
