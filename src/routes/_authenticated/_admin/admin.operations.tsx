...
import { Plus, Pencil, X, Check, Send, Eye, ShieldCheck, Upload, Wallet, ClipboardCheck, Siren, Search } from "lucide-react";
...
const blankCampaign = (): CampaignForm => ({
  name: "", category_id: "", description: "", location: "", city: "", barangay: "",
  severity: "moderate", status: "active",
  affected_families: "", affected_individuals: "", required_funding: "",
...
function Operations() {
  ...
  const allocs = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("fund_allocations").select("*").order("created_at", { ascending: false })).data ?? [],
  });

  const requests = useQuery({
    queryKey: ["ops-requests"],
    queryFn: async () => {
      const { data } = await supabase.from("fund_requests").select("*,disaster_categories(name)").order("created_at", { ascending: false });
      const rows = data ?? [];
  ...
  useEffect(() => {
    const ch = supabase
      .channel("ops-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "fund_requests" }, () => qc.invalidateQueries({ queryKey: ["ops-requests"] }))
      .on("postgres_changes", { event: "*", schema: "public", table: "disasters" }, () => qc.invalidateQueries({ queryKey: ["ops-disasters"] }))
      .on("postgres_changes", { event: "*", schema: "public", table: "fund_allocations" }, () => qc.invalidateQueries({ queryKey: ["ops-allocations"] }))
      .subscribe();
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((d: any) => `${d.name} ${d.city} ${d.barangay ?? ""}`.toLowerCase().includes(q));
    }
    return list;
  }, [campaigns.data, filter, search]);

  const unassignedRequests = useMemo(
    () => (requests.data ?? []).filter((r: any) => !r.disaster_id && r.status !== "rejected" && r.status !== "released"),
    [requests.data],
  );
  ...
  const saveCampaign = async () => {
    ...
    if (!e.name || !e.category_id || !e.location || !e.city || !e.occurred_at) return toast.error("Please fill required fields");
    ...
    if (Number.isNaN(families) || families < 0) return toast.error("Affected families must be a valid number");
    if (Number.isNaN(individuals) || individuals < 0) return toast.error("Affected individuals must be a valid number");
    if (Number.isNaN(funding) || funding < 0) return toast.error("Required funding must be a valid number");
    const payload = {
      name: e.name, category_id: e.category_id, description: e.description || null,
      location: e.location, city: e.city, barangay: e.barangay || null,
      severity: e.severity, status: e.status,
      affected_families: families, affected_individuals: individuals, required_funding: funding,
  ...
  const closeCampaign = async (id: string) => {
    if (!confirm("Close this campaign? It will no longer accept new donations or requests.")) return;
  ...
  const createAllocation = async () => {
    ...
    if (error) return toast.error(error.message);
    await logAudit("allocation.create", "fund_allocations", data?.id, { ...allocForm, disaster_id: allocFor.id });
  ...
  const allocationsForRelease = useMemo(() => {
    if (!releaseFor) return [];
    const tied = (allocs.data ?? []).filter((a: any) => !a.disaster_id || a.disaster_id === releaseFor.disaster_id);
    return tied;
    ...
    >
      ...
      {unassignedRequests.length > 0 && (
        <div className="mb-6 rounded-xl border border-warning/40 bg-warning/10 p-4">
          ...
          <p className="mb-3 text-xs text-muted-foreground">Approving one of these will automatically open a new disaster campaign so citizens can donate.</p>
      ...
      <Accordion type="multiple" className="space-y-3">
        {filteredCampaigns.map((d: any) => {
          ...
          const pendingCount = reqs.filter((r) => r.status === "pending" || r.status === "under_review").length;
          ...
          return (
            <AccordionItem key={d.id} value={d.id} className="overflow-hidden rounded-xl border border-border bg-card">
              <AccordionTrigger className="px-4 py-3 hover:no-underline">
                <div className="flex flex-1 flex-col gap-2 pr-3 text-left">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    ...
                    <span className="tabular-nums">{formatPHP(raised)} raised of {formatPHP(required)}</span>
                    <span className="tabular-nums">Allocated {formatPHP(allocated)} · Available {formatPHP(available)}</span>
                  </div>
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                    <div className="h-full bg-gradient-to-r from-relief to-primary transition-all" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              </AccordionTrigger>
                ...
                <Tabs defaultValue="requests">
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                    <TabsList>
                      ...
                      <TabsTrigger value="allocations">Allocations ({camAllocs.length})</TabsTrigger>
                  ...
                  <TabsContent value="requests" className="mt-0">
                    {reqs.length === 0 ? (
                      <p className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">No requests for this campaign yet.</p>
                    ) : (
                      <div className="space-y-2">
                        {reqs.map((r) => (
                          <RequestRow key={r.id} r={r} onReview={() => setReviewing(r)} onRelease={() => {
                            setReleaseFor(r);
                  ...
                  <TabsContent value="allocations" className="mt-0">
                    {camAllocs.length === 0 ? (
                      <p className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">No allocations yet. Use “Allocate funds” to earmark money for this campaign.</p>
                    ) : (
                      <div className="overflow-hidden rounded-md border border-border">
                        <table className="w-full text-sm">
                          <thead className="bg-secondary text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                            <tr><th className="px-3 py-2">Label</th><th className="px-3 py-2 text-right">Allocated</th><th className="px-3 py-2 text-right">Released</th><th className="px-3 py-2 text-right">Available</th></tr>
                          </thead>
                            ...
                            {camAllocs.map((a) => {
                              ...
                              return (
                                <tr key={a.id}>
                                  ...
                                  <td className="px-3 py-2 text-right tabular-nums">{formatPHP(a.allocated_amount)}</td>
                                  <td className="px-3 py-2 text-right tabular-nums text-relief">{formatPHP(a.released_amount)}</td>
                                  <td className="px-3 py-2 text-right tabular-nums font-semibold">{formatPHP(avail)}</td>
                                </tr>
                              );
                            })}
                          </tbody>
                  ...
                  <TabsContent value="details" className="mt-0 space-y-2 text-sm">
                    ...
                    {d.description && (
                      <div>
                        ...
                        <p className="mt-1 whitespace-pre-wrap rounded-md border border-border bg-paper p-3 text-sm">{d.description}</p>
                      </div>
                    )}
                  </TabsContent>
                </Tabs>
      ...
      <Dialog open={!!editingCampaign} onOpenChange={(o) => !o && setEditingCampaign(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader><DialogTitle>{editingCampaign?.id ? "Edit campaign" : "Register new campaign"}</DialogTitle></DialogHeader>
          {editingCampaign && (
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Campaign name *" className="sm:col-span-2"><Input value={editingCampaign.name} onChange={(e) => setEditingCampaign({ ...editingCampaign, name: e.target.value })} /></Field>
              <Field label="Category *">
                <Select value={editingCampaign.category_id} onValueChange={(v) => setEditingCampaign({ ...editingCampaign, category_id: v })}>
                  <SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger>
                  <SelectContent>{(cats.data ?? []).map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Occurred on *"><Input type="date" value={editingCampaign.occurred_at} onChange={(e) => setEditingCampaign({ ...editingCampaign, occurred_at: e.target.value })} /></Field>
              <Field label="Severity">
                <Select value={editingCampaign.severity} onValueChange={(v: any) => setEditingCampaign({ ...editingCampaign, severity: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{["low", "moderate", "high", "critical"].map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Status">
                <Select value={editingCampaign.status} onValueChange={(v: any) => setEditingCampaign({ ...editingCampaign, status: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{["active", "monitoring", "closed"].map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="City *"><Input value={editingCampaign.city} onChange={(e) => setEditingCampaign({ ...editingCampaign, city: e.target.value })} /></Field>
              <Field label="Barangay"><Input value={editingCampaign.barangay} onChange={(e) => setEditingCampaign({ ...editingCampaign, barangay: e.target.value })} /></Field>
              <Field label="Location / address *" className="sm:col-span-2"><Input value={editingCampaign.location} onChange={(e) => setEditingCampaign({ ...editingCampaign, location: e.target.value })} /></Field>
              <Field label="Description" className="sm:col-span-2"><Textarea rows={3} value={editingCampaign.description} onChange={(e) => setEditingCampaign({ ...editingCampaign, description: e.target.value })} /></Field>
              <Field label="Families affected"><Input inputMode="numeric" value={editingCampaign.affected_families} onChange={(e) => setEditingCampaign({ ...editingCampaign, affected_families: e.target.value.replace(/[^0-9]/g, "") })} /></Field>
              <Field label="Individuals affected"><Input inputMode="numeric" value={editingCampaign.affected_individuals} onChange={(e) => setEditingCampaign({ ...editingCampaign, affected_individuals: e.target.value.replace(/[^0-9]/g, "") })} /></Field>
              <Field label="Required funding (₱)" className="sm:col-span-2"><Input inputMode="decimal" value={editingCampaign.required_funding} onChange={(e) => setEditingCampaign({ ...editingCampaign, required_funding: e.target.value.replace(/[^0-9.]/g, "") })} /></Field>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingCampaign(null)}>Cancel</Button>
            <Button onClick={saveCampaign}>{editingCampaign?.id ? "Save changes" : "Create campaign"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!allocFor} onOpenChange={(o) => !o && setAllocFor(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>Allocate funds for {allocFor?.name}</DialogTitle></DialogHeader>
          <div className="grid gap-4">
            <div><Label className="text-xs">Label *</Label><Input className="mt-1" value={allocForm.label} onChange={(e) => setAllocForm({ ...allocForm, label: e.target.value })} placeholder="e.g. Phase 1 relief" /></div>
            <div><Label className="text-xs">Amount (₱) *</Label><Input className="mt-1" inputMode="decimal" placeholder="0.00" value={allocForm.allocated_amount} onChange={(e) => setAllocForm({ ...allocForm, allocated_amount: e.target.value.replace(/[^0-9.]/g, "") })} /></div>
            <div><Label className="text-xs">Notes</Label><Textarea className="mt-1" rows={3} value={allocForm.notes} onChange={(e) => setAllocForm({ ...allocForm, notes: e.target.value })} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAllocFor(null)}>Cancel</Button>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          {reviewing && (
            <div className="space-y-4 text-sm">
              <div className="grid gap-3 sm:grid-cols-2">
                <Info label="Requester">{reviewing.profiles?.first_name} {reviewing.profiles?.last_name}</Info>
                <Info label="Contact">{reviewing.profiles?.mobile_number} · {reviewing.profiles?.email}</Info>
                <Info label="Location">{reviewing.exact_location}, {reviewing.barangay}, {reviewing.city}</Info>
                ...
                <Info label="Requested amount" highlight>{formatPHP(reviewing.requested_amount)}</Info>
                <Info label="Status"><StatusBadge status={reviewing.status} /></Info>
              </div>
              <div>
                <Label className="text-xs uppercase tracking-wider text-muted-foreground">Description</Label>
                <p className="mt-1 whitespace-pre-wrap rounded-md border border-border bg-paper p-3 text-sm">{reviewing.disaster_description}</p>
              </div>
              <div>
          ...
          <DialogFooter className="flex-wrap gap-2">
            {reviewing && (
              <div className="mr-auto flex items-center gap-2 text-xs">
                ...
                {reviewing.verification_status !== "verified" && reviewing.status !== "released" && (
                  <Button variant="outline" size="sm" onClick={() => verifyRequest(reviewing)}><ShieldCheck className="h-3.5 w-3.5" /> Verify</Button>
                )}
              </div>
            )}
              ...
              <>
                ...
                {reviewing.status === "approved" && (
                  <Button onClick={() => {
                    ...
                    setReleaseForm({ amount: String(reviewing.requested_amount ?? ""), allocation_id: "", reference_number: "", notes: "" });
                    setReleaseProof(null);
                    setReviewing(null);
                  }}><Send className="h-4 w-4" /> Release funds</Button>
                )}
              </>
            )}
          </DialogFooter>
          ...
          {releaseFor && (
            <div className="grid gap-4 text-sm">
              <p className="rounded-md bg-secondary p-3 text-xs">
                For: <span className="font-medium">{releaseFor.profiles?.first_name} {releaseFor.profiles?.last_name}</span> · {formatPHP(releaseFor.requested_amount)} requested
              </p>
              <div><Label className="text-xs">Amount to release (₱) *</Label><Input className="mt-1" inputMode="decimal" placeholder="0.00" value={releaseForm.amount} onChange={(e) => setReleaseForm({ ...releaseForm, amount: e.target.value.replace(/[^0-9.]/g, "") })} /></div>
              <div>
                ...
                <label htmlFor="release-proof" className="mt-1 flex cursor-pointer items-center gap-3 rounded-lg border border-dashed border-border bg-paper p-3 text-xs hover:border-primary/40">
                  {releaseProof ? (
                    ...
                    <>
                      ...
                      <span className="text-muted-foreground">Upload receipt or signed acknowledgment (JPG, PNG, PDF — max 10MB)</span>
                    </>
                  )}
                </label>
...
function RequestRow({ r, onReview, onRelease, compact }: { r: any; onReview: () => void; onRelease?: () => void; compact?: boolean }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-md border border-border bg-card px-3 py-2 text-sm">
      <div className="min-w-0 flex-1">
        <p className="truncate font-medium">{r.profiles?.first_name} {r.profiles?.last_name} <span className="ml-1 text-xs text-muted-foreground">· {timeAgo(r.created_at)}</span></p>
        <p className="truncate text-xs text-muted-foreground">{r.barangay}, {r.city} {compact && r.disaster_categories?.name ? `· ${r.disaster_categories.name}` : ""}</p>
      </div>
      ...
      <div className="flex gap-1">
        <Button variant="ghost" size="sm" onClick={onReview}><Eye className="h-3.5 w-3.5" /> Review</Button>
        {r.status === "approved" && onRelease && (
          <Button variant="relief" size="sm" onClick={onRelease}><Send className="h-3.5 w-3.5" /> Release</Button>
        )}
      </div>
    </div>
  );
...
function StatusBadge({ status }: { status: string }) {
  const m: Record<string, string> = {
    pending: "bg-warning/15 text-warning-foreground", under_review: "bg-primary/10 text-primary",
    approved: "bg-relief/15 text-relief", rejected: "bg-destructive/15 text-destructive", released: "bg-gold/20 text-ink",
    active: "bg-relief/15 text-relief", monitoring: "bg-warning/15 text-warning-foreground", closed: "bg-secondary text-muted-foreground",
  };
