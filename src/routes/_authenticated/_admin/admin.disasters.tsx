...
function ManageDisasters() {
  ...
  const close = async (id: string) => {
    if (
      !confirm("Close this disaster campaign? It will no longer accept new donations or requests.")
    )
      return;
      ...
      actions={
        <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
          <DialogTrigger asChild>
            <Button variant="default" onClick={() => setEditing(blank())}>
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                {editing?.id ? "Edit disaster campaign" : "Register new disaster campaign"}
              </DialogTitle>
            </DialogHeader>
            {editing && (
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Severity">
                  <Select
                    ...
                    onValueChange={(v: any) => setEditing({ ...editing, severity: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <Field label="Barangay">
                  <Input
                    value={editing.barangay}
                    onChange={(e) => setEditing({ ...editing, barangay: e.target.value })}
                  />
                </Field>
                ...
                <Field label="Families affected">
                  <Input
                    ...
                    inputMode="numeric"
                    placeholder="0"
                    value={editing.affected_families}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                ...
                <Field label="Individuals affected">
                  <Input
                    ...
                    inputMode="numeric"
                    placeholder="0"
                    value={editing.affected_individuals}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                ...
                <Field label="Required funding (₱)" className="sm:col-span-2">
                  <Input
                    ...
                    inputMode="decimal"
                    placeholder="0.00"
                    value={editing.required_funding}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
            ...
            {(list.data ?? []).length === 0 && (
              <tr>
                <td colSpan={7} className="p-10 text-center text-muted-foreground">
                  No disaster campaigns registered yet.
                </td>
              </tr>
            )}
            {(list.data ?? []).map((d: any) => (
              <tr key={d.id} className="hover:bg-secondary/50">
                ...
                <td className="px-4 py-3 text-right tabular-nums">
                  ...
                  <div className="text-xs text-muted-foreground">
                    of {formatPHP(d.required_funding)}
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    ...
                    {d.status !== "closed" && (
                      <Button variant="ghost" size="sm" onClick={() => close(d.id)}>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${m[status]}`}
    >
      {status}
    </span>
  );
