...
function ManageDisasters() {
  ...
  const save = async () => {
    if (!editing) return;
    if (!editing.name || !editing.category_id || !editing.location || !editing.city || !editing.occurred_at) {
      toast.error("Please fill required fields");
      return;
    }
    const families = Number(editing.affected_families);
    const individuals = Number(editing.affected_individuals);
    const funding = Number(editing.required_funding);
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
            ...
            {editing && (
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Families affected"><Input type="text" inputMode="numeric" placeholder="0" value={editing.affected_families} onChange={(e) => setEditing({ ...editing, affected_families: e.target.value.replace(/[^0-9]/g, "") })} /></Field>
                <Field label="Individuals affected"><Input type="text" inputMode="numeric" placeholder="0" value={editing.affected_individuals} onChange={(e) => setEditing({ ...editing, affected_individuals: e.target.value.replace(/[^0-9]/g, "") })} /></Field>
                <Field label="Required funding (₱)" className="sm:col-span-2"><Input type="text" inputMode="decimal" placeholder="0.00" value={editing.required_funding} onChange={(e) => setEditing({ ...editing, required_funding: e.target.value.replace(/[^0-9.]/g, "") })} /></Field>
              </div>
            )}
            <DialogFooter>
