...
function AdminOverview() {
    ...
    queryFn: async () => {
      const [
        ...
        supabase
          .from("disasters")
          .select(
            "id,name,city,severity,affected_families,required_funding,raised_amount,status,disaster_categories(name),occurred_at,created_at",
          )
          .eq("status", "active")
        ...
        supabase
          .from("fund_requests")
          .select("id,disaster_description,requested_amount,status,created_at")
          .order("created_at", { ascending: false })
          .limit(5),
        supabase
          .from("donations")
          .select(
            "id,amount,created_at,donor_name,donor_email,is_anonymous,disaster_id,disasters(name)",
          )
          .order("created_at", { ascending: false })
          .limit(5),
      ]);
  ...
  const [iSort, setISort] = useState<"date_desc" | "date_asc" | "name_asc" | "name_desc">(
    "date_desc",
  );
  ...
  return (
    <AdminShell
      ...
      subtitle="A focused snapshot of fund operations and recent citizen donations."
    >
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi
          label="Donations received"
          value={formatPHP(d?.totalD ?? 0)}
          accent="relief"
          icon={HandHeart}
        />
        ...
        <Kpi
          ...
          value={String(d?.activeDisasters ?? 0)}
          icon={Siren}
          accent="primary"
        />
        <Kpi
      ...
      <section className="mt-8 min-w-0 overflow-hidden rounded-xl border border-border bg-card">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground">
                {activeFiltered.length} of {activeAll.length}
              </span>
              <Button variant="ghost" size="sm" className="h-8" onClick={resetActiveFilters}>
                Reset
              </Button>
            </div>
          </div>
        ...
        <ul className="divide-y divide-border">
          {activeAll.length === 0 && (
            <li className="p-8 text-center text-sm text-muted-foreground">No active campaigns.</li>
          )}
          {activeAll.length > 0 && activeFiltered.length === 0 && (
          ...
          {activeFiltered.map((row: any) => {
            const pct =
              row.required_funding > 0
                ? Math.min(100, (Number(row.raised_amount) / Number(row.required_funding)) * 100)
                : 0;
              ...
              <li key={row.id} className="grid gap-3 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                <div className="min-w-0">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    {row.disaster_categories?.name}
                  </p>
                  <p className="mt-0.5 font-medium">{row.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {row.city} · {row.affected_families.toLocaleString()} families ·{" "}
                    {timeAgo(row.occurred_at ?? row.created_at)}
                  </p>
                  <div className="mt-2 max-w-md">
                    <div className="flex items-baseline justify-between text-xs">
                      <span className="text-muted-foreground">Funding</span>
                      <span className="font-semibold tabular-nums">
                        {formatPHP(row.raised_amount)} /{" "}
                        {formatPHP(row.required_funding, { compact: true })}
                      </span>
                    </div>
      ...
      <section className="mt-8 min-w-0 overflow-hidden rounded-xl border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border p-5">
          <h2 className="font-display text-base font-semibold">
            Inactive / closed disaster campaigns
          </h2>
          <Button variant="ghost" size="sm" asChild>
            <Link to="/admin/operations">
              Manage <ArrowRight className="h-3 w-3" />
        ...
        <div className="border-b border-border bg-muted/30 p-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Input
              type="date"
              value={iFrom}
              onChange={(e) => setIFrom(e.target.value)}
              className="h-9"
              aria-label="From date"
            />
            <Input
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground">
                {inactiveFiltered.length} of {inactiveAll.length}
              </span>
              <Button variant="ghost" size="sm" className="h-8" onClick={resetInactiveFilters}>
                Reset
              </Button>
            </div>
          </div>
        ...
        <ul className="divide-y divide-border">
          ...
          {inactiveAll.length > 0 && inactiveFiltered.length === 0 && (
            <li className="p-8 text-center text-sm text-muted-foreground">
              No campaigns match these filters.
            </li>
          )}
      ...
      <div className="mt-8 grid items-start gap-6 lg:grid-cols-2">
        <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card">
          ...
          <ul className="divide-y divide-border">
            {(d?.recentReqs ?? []).length === 0 && (
              <li className="p-8 text-center text-sm text-muted-foreground">No requests yet.</li>
            )}
        ...
        <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card">
          ...
          <ul className="divide-y divide-border">
            {(d?.recentDonations ?? []).length === 0 && (
              <li className="p-8 text-center text-sm text-muted-foreground">No donations yet.</li>
            )}
                ...
                <li key={don.id} className="flex items-center justify-between gap-3 p-4 text-sm">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {campaign} · {timeAgo(don.created_at)}
                    </p>
                  </div>
                  <span className="shrink-0 text-right font-semibold tabular-nums text-relief">
                    {formatPHP(don.amount)}
                  </span>
                </li>
              );
            })}
...
function Kpi({
  ...
  label: string;
  value: string;
  accent?: "relief" | "gold" | "warning" | "primary";
  icon: any;
}) {
  const bar =
    accent === "relief"
