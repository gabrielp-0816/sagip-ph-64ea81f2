...
export const Route = createFileRoute("/admin-auth/")({
  head: () => ({
    meta: [{ title: "Admin sign in — SAGIP" }, { name: "robots", content: "noindex" }],
  }),
      ...
      <div className="hidden md:block">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-10">
          ...
          <h1 className="mt-3 font-display text-3xl font-semibold leading-tight">
            SAGIP Administrator Portal
          </h1>
          <p className="mt-4 text-sm text-paper/70">
            For authorized SAGIP DRRM personnel only. All actions performed inside the admin console
            are recorded in an immutable audit log.
          </p>
          <div className="mt-8 flex items-start gap-3 rounded-lg border border-gold/30 bg-gold/10 p-4 text-xs text-paper/80">
            ...
            <p>
              Unauthorized access attempts are logged and may be subject to administrative and legal
              action under RA 10173.
            </p>
          </div>
        </div>
      </div>
      <div>
        <div className="mx-auto max-w-md rounded-2xl bg-paper p-8 text-foreground shadow-2xl">
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Have an invite code?{" "}
            <Link to="/admin-auth/signup" className="font-medium text-primary hover:underline">
